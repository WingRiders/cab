...
export const isTxStakingKeyRegistrationCert = (value: any): value is TxStakingKeyRegistrationCert => {
  ...
  return type === TxCertificateType.STAKING_KEY_REGISTRATION && Buffer.isBuffer(pubKeyHash)
}
...
export const isStakingKeyDeregistrationCert = (value: any): value is TxStakingKeyDeregistrationCert => {
  ...
  if (!Array.isArray(value[1]) || value[1].length !== 2) return false
  const [type, [, pubKeyHash]] = value
  return type === TxCertificateType.STAKING_KEY_DEREGISTRATION && Buffer.isBuffer(pubKeyHash)
}
...
export const isDelegationCert = (value: any): value is TxDelegationCert => {
  ...
  return (
    type === TxCertificateType.DELEGATION && Buffer.isBuffer(pubKeyHash) && Buffer.isBuffer(poolHash)
  )
}
...
export const isTxSingleHostNameRelay = (value: any): value is TxSingleHostNameRelay => {
  ...
  return (
    type === TxRelayTypes.SINGLE_HOST_NAME && Number.isInteger(portNumber) && typeof dnsName === 'string'
  )
}
